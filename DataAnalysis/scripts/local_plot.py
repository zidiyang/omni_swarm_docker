...
def plot_fused(poses, poses_fused, poses_vo, poses_path, loops, detections, nodes, groundtruth = True, \
    ...
    plt.legend()
    plt.savefig(output_path+"Traj2.png")
    ...
    for k in range(len(nodes)):
        ...
        ax.set_ylabel('$Y$')
        ax.set_zlabel('$Z$')
    plt.savefig(output_path+"FusedVsGT3D.png")
    ...
    plt.legend()
    plt.grid()
    plt.savefig(output_path+"fused2d.png")
    ...
    for k in range(len(nodes)):
        ...
        plt.savefig(output_path+f"fusedvsgt2d_{i}.png")
    if not plot_each:
        return
    for i in nodes:
        _id = id_map[i]
        ...
        #error plot
        fig = plt.figure(f"Drone {i} fused compare GT Pos error", figsize=figsize)

        fig.suptitle(f"est_err_by_t{i}_position")
        ax1, ax2, ax3 = fig.subplots(3, 1)
        t_ = poses_fused[i]["t"]
        if groundtruth:
            pos_gt =  poses[i]["pos_func"](poses_fused[i]["t"])
        ...
        #if groundtruth:
            #ax1.plot(t_, pos_gt[:,0], label=f"Ground Truth ${i}$")
        #print(f"RMSE_pos: {RMSE_pos}")
        pos_vo = poses_vo[i]["pos_func"](poses_fused[i]["t"])
        #print(f"size of pos_vo {pos_vo.shape}")
        ax1.plot(poses_fused[i]["t"], abs(pos_vo[:,0] - pos_gt[:,0]), label=f"Aligned VO Traj{_id}")
        ax1.plot(poses_fused[i]["t"], abs(poses_fused[i]["pos"][:,0] - pos_gt[:,0]), label=f"Estimate {_id}")
        if use_offline:
            ax1.plot(poses_path[i]["t"], poses_path[i]["pos"][:,0], '.', label=f"Fused Offline Traj{i}")
        ax1.tick_params( axis='x', which='both', bottom=False, top=False, labelbottom=False) 
        ax1.set_ylabel("x")
        ...
        #if groundtruth:
            #ax2.plot(t_, pos_gt[:,1], label=f"Ground Truth ${i}$")
        ax2.plot(poses_fused[i]["t"], abs(pos_vo[:,1] - pos_gt[:,1]), label=f"Aligned VO Traj{_id}")
        ax2.plot(poses_fused[i]["t"], abs(poses_fused[i]["pos"][:,1] - pos_gt[:,1]), label=f"Estimate {_id}")
        if use_offline:
            ax2.plot(poses_path[i]["t"], poses_path[i]["pos"][:,1], '.', label=f"Fused Offline Traj{i}")
        ax2.tick_params( axis='x', which='both', bottom=False, top=False, labelbottom=False) 
        ax2.set_ylabel("y")
        ...
        #if groundtruth:
            #ax3.plot(t_, pos_gt[:,2], label=f"Ground Truth ${i}$")
        ax3.plot(poses_fused[i]["t"], abs(pos_vo[:,2] - pos_gt[:,2]), label=f"Aligned VIO ${_id}$")
        ax3.plot(poses_fused[i]["t"], abs(poses_fused[i]["pos"][:,2] - pos_gt[:,2]), label=f"Estimate {_id}")
        if use_offline:
            ax3.plot(poses_path[i]["t"], poses_path[i]["pos"][:,2], '.', label=f"Fused Offline Traj{i}")
        ax3.set_ylabel("z")
        ax3.set_xlabel("t")
        ...
        plt.savefig(output_path+f"est_err_by_t{i}_position.png")
        ...
        fig.suptitle(f"est_err_by_t{i}_attitude")
        #ax1, ax2, ax3 = fig.subplots(3, 1)
        yaw_vo =  poses_vo[i]["ypr_func"](poses_fused[i]["t"])[:, 0]
        pitch_vo =  poses_vo[i]["ypr_func"](poses_fused[i]["t"])[:, 1]
        roll_vo =  poses_vo[i]["ypr_func"](poses_fused[i]["t"])[:, 2]
        #if groundtruth:
            #ax1.plot(t_, yaw_gt*57.3, label=f"Ground Truth ${i}$")
        ax1.plot(t_, abs(yaw_vo*57.3 - yaw_gt*57.3), label=f"Aligned VIO ${_id}$")
        ax1.plot(poses_fused[i]["t"], abs(poses_fused[i]["ypr"][:,0]*57.3 - yaw_gt*57.3), label=f"Estimate {_id}")
        if use_offline:
            ax1.plot(poses_path[i]["t"], poses_path[i]["ypr"][:,0]*57.3, '.', label=f"EstKF {_id}")
        ...
        #if groundtruth:
            #ax2.plot(t_, pitch_gt*57.3, label=f"Ground Truth ${i}$")
        ax2.plot(t_, abs(pitch_vo*57.3 - pitch_gt*57.3), label=f"Aligned VIO ${_id}$")
        ax2.plot(poses_fused[i]["t"], abs(poses_fused[i]["ypr"][:,1]*57.3 - pitch_gt*57.3), label=f"Estimate {_id}")
        if use_offline:
            ax2.plot(poses_path[i]["t"], poses_path[i]["ypr"][:,1]*57.3, '.', label=f"EstKF {_id}")
        ...
        #if groundtruth:
            #ax3.plot(t_, roll_gt*57.3, label=f"Ground Truth ${i}$")
        ax3.plot(t_, abs(roll_vo*57.3 - roll_gt*57.3), label=f"Aligned VIO ${_id}$")
        ax3.plot(poses_fused[i]["t"], abs(poses_fused[i]["ypr"][:,2]*57.3 - roll_gt*57.3), label=f"Estimate {_id}")
        if use_offline:
            ax3.plot(poses_path[i]["t"], poses_path[i]["ypr"][:,2]*57.3, '.', label=f"EstKF {_id}")
        ax3.set_xlabel("t")
        ax3.legend()
        ...
        plt.savefig(output_path+f"est_err_by_t{i}_attitude.png")
        RMSE_pos = 0
        for j in range(0,t_.size):
            norm_err = norm(poses_fused[i]["pos"][j,:] - pos_gt[j,:])
            RMSE_pos = RMSE_pos+norm_err
        RMSE_pos = RMSE_pos/t_.size
        RMSE_att = 0
        att_gt =  poses[i]["ypr_func"](poses_fused[i]["t"])
        for j in range(0,t_.size):
            norm_att_err = norm(poses_fused[i]["ypr"][j,:] - att_gt[j,:])*57.3
            #print(f"norm{norm_att_err}")
            RMSE_att = RMSE_att+norm_att_err
        RMSE_att = RMSE_att/t_.size
        print(f"RMSE of pos: {RMSE_pos}")
        print(f"RMSE of rot: {RMSE_att}")
    ...
    for main_id in nodes:
        for i in nodes:
            ...
            if is_show:
                ...
                ax2.plot(t_, abs(dis_gt-dis_raw), ".", label="Distance Error of UWB")
                ax2.plot(t_, abs(dis_gt-dis_fused), label="Distance Error of Fused")
            ...
            if is_show:
                ...
                plt.title(title)
                plt.savefig('dist_hist_error.png')
                plt.show()
        break
    ...
    for target_id in target_ids:
        ...
        if show:
            ...
            if target_id == target_ids[0]:
                plt.grid()
            plt.savefig('Relative Pose 2D.png')
            ...
            ax2.grid()
            plt.tight_layout()
            plt.savefig('Relative Pose PolarErr.png')
            ...
            ax4.grid()
            plt.tight_layout()
            plt.savefig('Relative Pose.png')
                
            fig, (ax1, ax2, ax3, ax4) = plt.subplots(4, 1, figsize=figsize)
            fig.suptitle(f"Fused Relative Error {main_id}->{target_ids}")
            #ax1, ax2, ax3 = fig.subplots(3, 1)

            ax1.plot(ts[mask], abs(dp_gt[mask,0] - dp_fused[mask,0]), label="$E_{xfused}^" + str(target_id) + f"$ RMSE:{rmse_x:3.3f}")
            ax2.plot(ts[mask], abs(dp_gt[mask,1] - dp_fused[mask,1]), label="$E_{yfused}^" + str(target_id) + f"$ RMSE:{rmse_y:3.3f}")
            ax3.plot(ts[mask], abs(dp_gt[mask,2] - dp_fused[mask,2]), label="$E_{zfused}^" + str(target_id) + f"$ RMSE:{rmse_z:3.3f}")
            ax4.plot(ts[mask], abs(wrap_pi(dyaw_gt[mask] - dyaw_fused[mask])), label="$E_{yawfused}^" + str(target_id) + f"$ RMSE:{rmse_z:3.3f}")
            ...
            ax4.grid()
            plt.tight_layout()
            plt.savefig('Fused Relative Error.png')
...
def plot_loops_error(poses, loops, good_loop_id=None, outlier_show_thres=0.5, show_outlier=True):
    ...
    plt.grid(which="both")
    plt.legend()
    plt.savefig('loop_pos_error.png')
    # plt.subplot(313)
    ...
    plt.grid(which="both")
    plt.savefig('Distance_vs_PosErr.png')
    ...
    plt.savefig('Dt_vs_YawErr_(deg).png')
